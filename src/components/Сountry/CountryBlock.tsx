...
export default function CountryBlock({
  ...
  const { ref: blockRef, isInView: blockInView } = useInViewAnimation<HTMLDivElement>();
  ...
  return (
    <div
      ref={blockRef}
      className={`${reversed ? s.country_block_reversed : s.country_block} ${blockInView ? s.animate : ''} ${dynamicClass}`}
    >
      {year && (
